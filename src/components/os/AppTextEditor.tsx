...
const AppTextEditor = ({ initialFilePath }: AppTextEditorProps) => {
  ...
  const saveFile = useCallback(() => {
    if (!activeTab) return;
    if (!activeTab.path) {
      setSaveAsPath('/home/akib/Documents/' + activeTab.name);
      setShowSaveAs(true);
      return;
    }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-3 py-1.5 text-xs border-b border-os-panel-border bg-os-window-chrome">
        ...
        <button onClick={() => { setSaveAsPath(activeTab?.path || '/home/akib/Documents/untitled.txt'); setShowSaveAs(true); }} className="text-os-window-chrome-foreground opacity-70 hover:opacity-100">
          Save As
        </button>
      </div>
